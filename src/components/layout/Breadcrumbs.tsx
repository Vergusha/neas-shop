...
const Breadcrumbs: React.FC = () => {
  ...
  return (
    <div className={`container px-4 py-2 mx-auto ${currentTheme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>
      <div className="text-sm">
        <ul className="flex flex-row items-center">
          {breadcrumbs.map((breadcrumb, index) => (
            <li key={breadcrumb.path} className="flex items-center">
              {index > 0 && <ChevronRight size={14} className={`mx-1 ${currentTheme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`} />}
              {index < breadcrumbs.length - 1 ? (
                <Link 
                  to={breadcrumb.path} 
                  className={`${currentTheme === 'dark' ? 'text-gray-300 hover:text-[#95c672]' : 'text-gray-700 hover:text-[#003d2d]'}`}
                >
                  {breadcrumb.name}
                </Link>
              ) : (
                <span className={`${currentTheme === 'dark' ? 'text-gray-400' : 'text-gray-500'} truncate max-w-[300px]`}>
                  {breadcrumb.name}
                </span>
              )}
            </li>
