  ...
  return (
    ...
    >
      ...
      <button 
        type="button"
        onClick={prevImage}
        className={`absolute p-1 transform -translate-y-1/2 rounded-full shadow-md left-2 top-1/2 ${
          currentTheme === 'dark' ? 'bg-gray-800/70 hover:bg-gray-800' : 'bg-white/70 hover:bg-white'
        }`}
        aria-label="Previous image"
      >
        <ChevronLeft size={16} className={currentTheme === 'dark' ? 'text-gray-300' : 'text-gray-700'} />
      </button>
      <button 
        type="button"
        onClick={nextImage}
        className={`absolute p-1 transform -translate-y-1/2 rounded-full shadow-md right-2 top-1/2 ${
          currentTheme === 'dark' ? 'bg-gray-800/70 hover:bg-gray-800' : 'bg-white/70 hover:bg-white'
        }`}
        aria-label="Next image"
      >
        <ChevronRight size={16} className={currentTheme === 'dark' ? 'text-gray-300' : 'text-gray-700'} />
      </button>
        ...
        {validImages.map((_, idx) => (
          <button
            ...
            className={`w-2 h-2 rounded-full ${
              idx === currentIndex 
                ? currentTheme === 'dark' ? 'bg-[#95c672]' : 'bg-[#003D2D]'
